...
export default function UmumPage() {
  ...
  const [student, setStudent] = useState({ name: '', school: '' }); // Empty default school
  ...
  // Load state on mount
  useEffect(() => {
    const savedState = localStorage.getItem('current_quiz_state_umum');
    if (savedState) {
  ...
  useEffect(() => {
    if (started && !submitted) {
      ...
      localStorage.setItem('current_quiz_state_umum', JSON.stringify(stateToSave));
    }
  ...
  const sendWhatsApp = () => {
    const message = `Halo Guru, saya *${student.name}* dari *${student.school}* telah menyelesaikan latihan soal *${getSubjectName()}* dengan nilai *${calculateScore()}*.`;
      ...
      <header className={styles.header}>
        <div className={styles.headerGrid}>
          ...
          <div className={styles.headerText}>
            ...
            <h2 className={styles.schoolName}>{student.school || 'PILIH SEKOLAH'}</h2>
          </div>
        </div>
        {started && !submitted && (
          ...
          {!submitted ? (
            ...
            <div className={`${styles.card} animated-fade-in`} style={{ textAlign: 'center' }}>
              ...
              <div style={{ margin: '1rem 0', color: '#8b949e' }}>
                ...
                <p><strong>Sekolah:</strong> {student.school}</p>
